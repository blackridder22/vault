...
function globalIgnorePath() {
    return path.join(configDir(), "ignore.txt");
}
function toPosix(relativePath) {
    return relativePath.replace(/\\/g, "/");
}
function displayHome(filePath) {
...
async function ensureGlobalIgnoreFile() {
...
async function loadIgnoreFile(ignorePath) {
...
async function createCopyFilter(sourceRoot) {
    ...
    return async (sourcePath) => {
        ...
        return !(isIgnored(globalMatcher, normalizedPath, stats.isDirectory()) ||
            isIgnored(projectMatcher, normalizedPath, stats.isDirectory()));
    };
}
async function backupProject(inputPath) {
